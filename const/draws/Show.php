            ...
            if ($GLOBALS['streaming'] == "1") {
              try {
                ...
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
              	...
              	$stmt = $conn->prepare("SELECT * FROM tbl_episodes WHERE item = ? AND streaming = '1' AND episode_no = ? AND size <= ? ORDER BY size");
              	$stmt->execute([$item_id, $ce, $max_vid_size]);
              	$result = $stmt->fetchAll();

                foreach($result as $row)
                {
                  ?><source src="<?php echo $row[2]; ?>" type="video/mp4" size="<?php echo $row[3]; ?>"><?php
              	}

                $stmt = $conn->prepare("SELECT * FROM tbl_episode_subs WHERE item = ? AND episode = ? ORDER BY language");
                ...
                foreach($result as $row)
                {
                  ?><track kind="captions" label="<?php echo $row[3]; ?>" srclang="<?php echo $row[4]; ?>" src="<?php echo $row[2]; ?>" <?php if ($sstart == "0") { print ' default ';} ?>><?php
                  $sstart++;
                }
